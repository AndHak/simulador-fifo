...
interface SimulationProcessProps {
    ...
    onReiniciar?: (p: Process) => void;
    onLimpiarRegistro?: () => void;
...
const Row = memo(function Row({
    ...
    const showNum = (v?: number | null) => (typeof v === "number" ? v : "-");
    const tiempoEjecutado = typeof p.iteracion === "number"
        ? p.iteracion
        : typeof p.tiempo_total === "number" && typeof p.tiempo_restante === "number"
            ? Math.max(0, p.tiempo_total - p.tiempo_restante)
            : "-";
    return (
        <TableRow className={p.estado === "ejecutando" ? "bg-blue-50 dark:bg-blue-900/20" : ""}>
            ...
            <TableCell className="w-36">
                <div className="h-2 bg-muted rounded overflow-hidden">
                    <div
                        style={{ width: `${Math.max(0, Math.min(100, p.progreso ?? 0))}%` }}
            ...
            <TableCell className="w-10">
                <div className="flex justify-center">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="flex flex-col items-start gap-0.5" onPointerDown={e => e.stopPropagation()}>
                            {/* Listo */}
                            <Button
                            ...
                            {/* Reanudar (only when suspended) */}
                            {p.estado === "suspendido" && (
                                <Button
                                    variant="ghost"
                                    className="w-full justify-start"
                                    onClick={() => {
                                        onActualizar(p.pid, { estado: "listo", interactividad: 2 });
                                        onReanudar?.(p.pid);
                                    }}
                                >
    ...
    return (
        ...
        a.estado === b.estado &&
        (typeof a.progreso === "number" ? a.progreso : 0) === (typeof b.progreso === "number" ? b.progreso : 0)
    );
});
...
export default function SimulationProcess({
    ...
    // ----- derived lists -----
    const executing = useMemo(() => procesos.find(p => p.estado === "ejecutando") ?? null, [procesos]);
    const suspended = useMemo(() => procesos.filter(p => p.estado === "suspendido"), [procesos]);
    const ready = useMemo(() => procesos.filter(p => p.estado === "listo"), [procesos]);
    ...
    return (
        <>
            ...
            {(registroOpen || registro.length > 0) && (
                <div className={`fixed inset-0 z-40 flex items-center justify-center p-6 ${registroOpen ? "block" : "hidden"}`}>
                    ...
                    <div className="relative z-50 w-full max-w-5xl bg-background rounded-lg shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
                        <div className="p-6 border-b flex items-center justify-between bg-gradient-to-r from-primary/10 to-primary/5">
                            <div>
                                <h3 className="text-xl font-bold text-foreground">Registro de Procesos Terminados</h3>
                                <p className="text-sm text-muted-foreground mt-1">Historial completo de ejecución</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="px-3 py-1.5 bg-primary/10 rounded-full">
                                    <span className="text-sm font-semibold text-primary">
                                        {registro.length} {registro.length === 1 ? 'proceso' : 'procesos'}
                                    </span>
                                </div>
                                {onLimpiarRegistro && registro.length > 0 && (
                                    <Button variant="destructive" size="sm" onClick={onLimpiarRegistro}>
                        ...
                        <div className="p-6 overflow-auto flex-1">
                            {registro.length === 0 ? (
                                <div className="text-center py-12">
                                    <p className="text-muted-foreground text-lg">No hay procesos terminados aún</p>
                                </div>
                            ) : (
                                <Table>
                                    <TableHeader>
                                        ...
                                        {registro.map(p => {
                                            ...
                                            return (
                                                <TableRow key={p.pid} className="hover:bg-muted/50">
                                                    ...
                                                    <TableCell><span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">{tInicio}s</span></TableCell>
                                                    <TableCell><span className="px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs font-medium">{tFinal}s</span></TableCell>
                                                    <TableCell><span className="font-semibold text-primary">{duracion}s</span></TableCell>
                                                    <TableCell>
                                                        <div className="flex justify-center">
                                                            <DropdownMenu>
                                                                <DropdownMenuTrigger asChild>
                                                                ...
                                                                <DropdownMenuContent className="flex flex-col items-start gap-0.5" onPointerDown={e => e.stopPropagation()}>
                                                                    ...
                                                                    <Button variant="ghost" className="w-full justify-start text-muted-foreground" onClick={() => onEditar?.(p)}>
